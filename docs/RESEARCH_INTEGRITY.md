...
- [x] Phase 3.5 keeps historical descriptive prices in `data/phase35/historical/` contracts and forward executable books in `data/phase35/forward/`; historical `p` is never labeled ask/bid/fill. Pre-registered checkpoints are exactly 48/24/12/6/3/1 hours. Readiness does not launch full 12-month collection or a forward daemon. Historical research readiness may use `survivorship_limited_descriptive` without proving universe completeness; forward observational readiness is distinct from `EXECUTABLE_TWO_SIDED_BOOK_VALIDATED`; fixture-only never establishes live forward readiness; `PHASE35_COLLECTION_READY` is not executable/profitable.
