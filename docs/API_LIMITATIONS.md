...
| Current order book | `GET https://clob.polymarket.com/book?token_id=` | **Current** book only. Phase 3.5 forward readiness may snapshot this via `ReadOnlyHttpClient` into `data/phase35/forward/` with immutable raw provenance. One-sided books (`ASK_ONLY` / `BID_ONLY`) are retained as valid market-state observations; empty/crossed/malformed fail closed. Two-sided executability is tracked separately and is not required for observational collection readiness. Hypothetical VWAP/depth are offline paper calculations; fee stays null/unknown unless externally sourced. No order submission. |
